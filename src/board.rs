...
pub type BitBoards = [BitBoard; 12];
...
mod tests {
    ...
    fn test_remove_piece() {
        let mut board = Board::empty_board();
        let pos = Coords { x: 0, y: 1 };

        // Place a white pawn at (0, 1) and verify
        board.place_piece(WHITE_PAWN, pos);
        assert_eq!(board.bit_boards[WHITE_PAWN], 1u64 << 8);

        // Remove the white pawn and verify
        board.remove_piece(WHITE_PAWN, pos);
        assert_eq!(board.bit_boards[WHITE_PAWN], 0);
    }

    #[test]
    fn test_print_board_no_panic() {
        // confirm no panic occurs when printing board
